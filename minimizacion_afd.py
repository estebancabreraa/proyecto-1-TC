...
def traduccionAFDMinimizado(afd):
    Panterior = []
    Pactual = []
...
def convertirAFDMinimizadoNodo(Mstates, afd):
    diccionarioAFDM = {}
    contador = 0
    nodo = Nodo('')
    ...
    for estadoAFDM in Mstates:
        for estadoInicial in afd.estadoInicial:
            if estadoInicial in estadoAFDM:
                estado = get_key(diccionarioAFDM, estadoAFDM)
            ...
            if transicion[0] in estadoAFDM:
                for estadoAFDM2 in Mstates:
                    if transicion[2] in estadoAFDM2: 
                        estadoI = get_key(diccionarioAFDM, estadoAFDM)
                        estadoF = get_key(diccionarioAFDM, estadoAFDM2)
